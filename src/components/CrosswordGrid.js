...
import { removeOneLetter } from './gameUtils'
              ...
              {Array.from({ length: gridSize }).map((_, colIndex) => {
                ...
                return (
                  <td key={`${rowIndex}-${colIndex}`} className='cell' onClick={() => handleCellClick(rowIndex, colIndex)}>
                    ...
                    <input ref={cellRefs.current[cellIndex]} type='text' maxLength='1' value={cellValues[cellIndex]} onChange={(e) => handleCellChange(cellIndex, e.target.value)} />
                  </td>
                )
              })}
            </tr>
